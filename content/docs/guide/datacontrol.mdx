...
<Callout type="info">
若恢复后未立即显示最新数据，可手动重启容器以确保缓存同步：`docker restart ech0`
</Callout>
...
CLI 工具同样适用于二进制安装用户，提供直接命令方式备份与恢复：
...
后台会自动根据此计划执行 backup 任务，并在日志中记录执行结果。系统在自动备份时会创建时间戳命名的快照文件，可通过 Web 面板查看或下载。
</Callout>
